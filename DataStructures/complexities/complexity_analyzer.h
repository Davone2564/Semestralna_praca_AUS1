    ...
    void ComplexityAnalyzer<Structure>::analyze(Structure structurePrototype)
    {
        ...
        for (int replication = 0; replication < this->getReplicationCount(); replication++) {
            ...
            for (int step = 0; step < this->getStepCount(); step++) {
                insertN_(structure, this->getStepSize());
                ...
                // vrati novo vytvoreny vektor(hranate zatvorky) a potom vlozi vysledok jedneho kroku do vytvoreneho vektora, viazaneho na dany kluc
                data[structureSize].push_back(result);
            }
