...
namespace ds::adt {
    ...
    size_t GeneralList<T, SequenceType>::calculateIndex(T element)
    {
        ...
        if (!this->getSequence()->findBlockWithProperty([&](auto block)
            {
    ...
    T GeneralList<T, SequenceType>::accessFirst()
    {
        ...
        if (!block) throw std::out_of_range("No first element in list!");
    ...
    T GeneralList<T, SequenceType>::access(size_t index)
    {
        ...
        if (!block) throw std::out_of_range("Wrong index!");
    ...
    {
        if (index < 0 || index > this->size()) throw std::out_of_range("Wrong index!");
